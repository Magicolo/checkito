...
- For generators with a small number of possible values (e.g., `bool`, a short range, or a tuple of small types), `{{ package.name }}` will automatically run exhaustively to cover all possible values. For larger or infinite input spaces, it samples randomly. This is meant as a complement to other testing strategies.
...
-   `Generate`: is implemented for many of rust's standard types and allows the generation of any random composite/structured data through combinator (such as tuples, `Any`, `Map`, `Flatten` and more). It is designed for composability and its usage should feel like working with `Iterator`s.
