...
fn main() {
    ...
    /// The general pattern for producing [`Node`]s is to generate the inner values for each enum case and map them
    /// to their corresponding [`Node`] constructor by using the [`Generate::map`] combinator.
    fn node() -> impl Generate<Item = Node> {
        (
            // [`with`] builds a generator based on the provided function.
            // An alternative would be to use `Same(Node::Null)`, but that would required a [`Clone`] implementation for [`Node`],
            // so the [`with`] solution is preferred.
            with(|| Node::Null),
            // Uses [`bool`]'s canonical [`Generate`] through its [`FullGenerate`] implementation.
            bool::generator().map(Node::Boolean),
            // [`number`] is a helper [`Generate`] implementation that produces non-infinite and non-NaN numbers.
            number::<f64>().map(Node::Number),
            string(),
            // [`lazy`] is a helper [`Generate`] implementation that prevents from recursing unconditionally (since it would blow up the stack).
            lazy(node)
                // [`Generate::collect_with`] will call the previous generator a number of time defined by the provided [`Generate<Item = usize>`].
                // [`Generate::dampen`] is used to prevent an exponential explosion of nodes by reducing the `size` of the [`Generate`] it is
                // applied to as recursion goes deeper. When the maximum depth is reached (see [`Generate::dampen_with`]), the `size` is set to 0.
                .collect_with((..32usize).dampen())
                .map(Node::Array)
                // [`Generate::boxed`] is used to make the return type finite. Without it, since the `impl Generate` type refers to itself
                // through the recursive calls to [`node`], the type never stabilizes.
                .boxed(),
            (string(), lazy(node))
                .collect_with((..32usize).dampen())
                .map(Node::Object)
